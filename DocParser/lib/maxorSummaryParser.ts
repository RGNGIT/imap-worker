...
const fetchOffset = (textArray, fromName, toName) => {
    let indexFrom = null, indexTo = null;
    for(let i = 0; i < textArray.length; i++) {
        if(textArray[i].text.includes(fromName)) {
            indexFrom = i;
        }
        if(textArray[i].text.includes(toName)) {
            indexTo = i;
        }
    }
...
class MaxorSummaryParser {
    ...
    async processAR01C(file, pages) {
        ...
        for await(const page of pages) {
            const fromBillToName = fetchOffset(page.texts, 'Bill To:', 'Billing Period');
