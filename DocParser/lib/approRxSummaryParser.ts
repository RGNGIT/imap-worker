...
function findValueByField(textArray, fieldName, offset?) {
    const defaultOffset = 1;
    for(let i = 0; i < textArray.length; i++) {
        if(textArray[i] === fieldName) {
            return textArray[i + defaultOffset + offset];
        }
    }
    return null;
}
...
async processFile(file) {
    ...
    */
    await seq.query(`INSERT INTO pbm.summary 
        ...
        VALUES ('${file.name}', 'ApproRx', '${await groupNumber(findValueByField(texts, 'Bill To:'))}', '${findValueByField(texts, 'Bill To:')}', '${texts[2].text}', '${texts[7].text.split('-')[0]}', '${texts[7].text.split('-')[1]}', '');`);
}
